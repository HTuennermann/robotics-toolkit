...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    d = ['x%d' % i for i in range(N)]
    d += ['y%d' % i for i in range(N)]
    d += ['lx%d' % i for i in range(num_landmarks)]
    d += ['ly%d' % i for i in range(num_landmarks)]

    d = [['x%d' % i, 'y%d' % i] for i in range(N)]
    d += [['lx%d' % i, 'ly%d' % i] for i in range(num_landmarks)]
    d = [item for sublist in d for item in sublist]

    m = {j: i for i, j in enumerate(d)}

    ma = [[0.0 for row in range(N * 2 + num_landmarks * 2)] for col in range(N * 2 + num_landmarks * 2)]
    xi = [[0.0] for row in range(N * 2 + num_landmarks * 2)]

    ma[m["x%d" % (0)]][m["x%d" % 0]] += 1
    ma[m["y%d" % (0)]][m["y%d" % 0]] += 1
    xi[m["x%d" % (0)]][0] += world_size / 2
    xi[m["y%d" % (0)]][0] += world_size / 2

    smotion = 1. / motion_noise
    smeasurement = 1. / measurement_noise

    for i, d in enumerate(data):

        for j in (d[0]):
            ma[m["x%d" % (i)]][m["x%d" % i]] += 1 * smeasurement
            ma[m["x%d" % (i)]][m["lx%d" % j[0]]] -= 1 * smeasurement

            ma[m["lx%d" % (j[0])]][m["x%d" % (i)]] -= 1 * smeasurement
            ma[m["lx%d" % (j[0])]][m["lx%d" % j[0]]] += 1 * smeasurement

            xi[m["x%d" % (i)]][0] -= j[1] * smeasurement
            xi[m["lx%d" % (j[0])]][0] += j[1] * smeasurement

            ma[m["y%d" % (i)]][m["y%d" % i]] += 1 * smeasurement
            ma[m["y%d" % (i)]][m["ly%d" % (j[0])]] -= 1 * smeasurement

            ma[m["ly%d" % (j[0])]][m["y%d" % (i)]] -= 1 * smeasurement
            ma[m["ly%d" % (j[0])]][m["ly%d" % (j[0])]] += 1 * smeasurement

            xi[m["y%d" % (i)]][0] -= j[2] * smeasurement
            xi[m["ly%d" % (j[0])]][0] += j[2] * smeasurement

        # movement part
        ma[m["x%d" % (i)]][m["x%d" % i]] += 1 * smotion
        ma[m["x%d" % (i)]][m["x%d" % (i + 1)]] -= 1 * smotion
        ma[m["x%d" % (i + 1)]][m["x%d" % (i)]] -= 1 * smotion
        ma[m["x%d" % (i + 1)]][m["x%d" % (i + 1)]] += 1 * smotion

        xi[m["x%d" % (i)]][0] -= d[1][0] * smotion
        xi[m["x%d" % (i + 1)]][0] += d[1][0] * smotion

        ma[m["y%d" % (i)]][m["y%d" % i]] += 1 * smotion
        ma[m["y%d" % (i)]][m["y%d" % (i + 1)]] -= 1 * smotion
        ma[m["y%d" % (i + 1)]][m["y%d" % (i)]] -= 1 * smotion
        ma[m["y%d" % (i + 1)]][m["y%d" % (i + 1)]] += 1 * smotion

        xi[m["y%d" % (i)]][0] -= d[1][1] * smotion
        xi[m["y%d" % (i + 1)]][0] += d[1][1] * smotion
