...
class PIDController:
    def __init__(self, p_gain, i_gain, d_gain, sense):
        self.s0 = sense
        self.i = 0
        self.p_gain = p_gain
        self.i_gain = i_gain
    ...
    def run(self, sense):
        p = -self.p_gain * sense
        d = -self.d_gain * (sense - self.s0)
        self.i += -self.i_gain * sense
        self.s0 = sense
        return p + d + self.i
...
def run(param1, param2, param3):
    ...
    pid = PIDController(param1, param3, param2, myrobot.y)


    for j in range(N*10):


        myrobot = myrobot.move(pid.run(myrobot.y), speed)
